...
env.SocketTestSTream.assignTimestampsAndWatermarks(new TimestampExtractor),
这个就是读取数据,就是监听Socket里面的数据。
在它后面,我们就通过assignTimestampsAndWatermarks这个方法来手动的去分配Watermark。
...
只是CEP它是一个面向问题的流处理技术, 它主要用于分析低延迟，来自不同数据源的事件流, 能够帮助我们在复杂的完全不相关的事件之中 找出它们之间有意义的联系,并且以接近实时来进行输出。
...
在明白了CEP技术是什么之后,
我们来了解一下FinkCEP。
FinkCEP就是在Fink之上的一个库。
这个库就是用来专门实现CEP技术的。
据我所知,到现在除了Fink其他的计算引擎,
例如Spark,它是没有提供专门的CEP库。
FinkCEP库它主要的组件有Event Stream,也就是事件流,
Pattern定义以及Pattern检测。
Pattern就是模式。模式是什么? 等下后面我会解释给大家听。
还有就是生成Alert。生成Alert就是生成警告。
FinkCEP一个非常重要的概念叫做Pattern。
